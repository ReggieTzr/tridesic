...
public:
    ...
    void March(double *SourceVal, double *DistanceData)
    {
        ...
        { /* Fill up the result matrix with data.                    */
            *(DistanceData++) = V[i].U;
        }
    }
...
protected:
    ...
    void MarchAlg(struct Vertex *temp_V,
                  int temp_Vnum,
                  double *srcval);
    void __forceinline CallTUpdateNeighbors(int             i,    /* vertex number */
    ...
    __forceinline double update(struct Stencil *current_stencil, int k, int l);
...
/***************************************************************************/
bool nextT(int ti, int v1, int v2, int *v3, int *tn,
           struct Triangle *T, struct Vertex *V, int Tnum)
{
    ...
    if (found)
    { /* find v3, the other vertex in the triangle to be unfolded.             */
        if (T[*tn].Vind[0] == v1)
        {
            if (T[*tn].Vind[1] == v2)
                *v3 = T[*tn].Vind[2];
...
/***************************************************************************/
int Split(int ti, int V0, int V1, int V2, struct Triangle *T, struct Vertex *V,
          int NonSplitTnum, int Vnum)
{
    ...
    while (nextT(tm, v1, v2, &v3, &tn, T, V, NonSplitTnum) and (not splitter))
    {
        ...
        if (x3 > 0 and y3 / x3 > ta)
        { /* if we found a splitter */
            splitter = true;
...
/* triangles.                                                               */
/***************************************************************************/
void FMM::InitGraph(struct Triangle *T, struct Vertex *V, int Vnum, int NonSplitTnum, int Tnum)
{
    ...
    for (i = 0; i < Vnum; i++)
    { /* scan all vertices */
        for (ti = 0; ti < V[i].ti; ti++)
        { /* scan connected triangles */
            if (V[i].Tind[ti] < NonSplitTnum)
            { /* if valid triangle */
                ...
                if (ca < 0)
                { /* If this triangle is an obtuse angle        */
                ...
                { /* If no splitting was nessesery create
                         ...
                         int Vnum, int Tnum)
{
    ...
    *V = (struct Vertex *)malloc(sizeof(struct Vertex) * Vnum);
    ...
    for (i = 0; i < Vnum; i++)
    { /* Add every triangle to its vertixes.    */
        (*V)[i].ti = 0;
    }
...
/***************************************************************************/
__forceinline bool Quadratic(double qa, double qb, double qc, double *x1)
{
...
/***************************************************************************/
__forceinline double FMM::update(struct Stencil *current_stencil, int k, int l)
{
    ...
    if (u >= 0)
    {
        ...
        if (Quadratic(current_stencil->shortcut1_2, /* If the quadratic equation has solutions */
                      ...
                      (u * u + current_stencil->shortcut2_2), &t) and
            (-(u + u) >= ctheta_mul_ma_div_mb_minus_one_mul_2 * t) and current_stencil->Ctheta_mul_l1_div_l2_minus_one_mul_2 * (t - u) <= (u + u))
            return (u1 + t);
    ...
    else
    {
        ...
        if (Quadratic(current_stencil->shortcut1_1, /* If the quadratic equation has solutions */
                      -u * ctheta_mul_ma_div_mb_minus_one_mul_2,
                      (u * u + current_stencil->shortcut2_1), &t) and
            (u + u >= ctheta_mul_ma_div_mb_minus_one_mul_2 * t) and current_stencil->Ctheta_mul_l2_div_l1_minus_one_mul_2 * (t + u) <= -(u + u))
            return (u2 + t);
        else /* If the quadratic equation has no solution,
                                                   or the solutions are out of the trinagle */
            return MAX_DISTANCE;
...
)
{
    ...
    for (st_ptr = V[i].ST, st_ind = V[i].si; st_ind > 0; st_ind--, st_ptr++)
    {
        ...
        k = st_ptr->v1;
        l = st_ptr->v2;
        if (k == becomes_alive || l == becomes_alive)
        {                            /* Use this stencil to update only if one k or
            ...
            if (heap->BP[k] == Alive || heap->BP[l] == Alive)
            { /* Do update only if the two other vertexes
...
/***************************************************************************/
void __forceinline FMM::CallTUpdateNeighbors(
    int i,           /* vertex number */
    int start_from /* The source from which the current march began.    */
)
{
...
double** get_fmm_geodesics(const double *X, const double *Y, const double *Z, const int *TRI, int vnum, int tnum){
    // create vars
    double input_dist[vnum][vnum], geod_dist[vnum][vnum];
    ...
    for (int i = 0; i < vnum; i++)
    {
        for (int j = 0; j < vnum; j++)
        {
            ...
            {
                input_dist[i][j] = MAX_DISTANCE;
            }
        }
    }

    FMM *fmm = new FMM((double *)X, (double *)Y, (double *)Z, (int *)TRI, vnum, tnum);

    int Snum = vnum; // Number of sources
    int Vnum = fmm->GetV();
    ...
    for (int k = 0; k < Snum; k++)
    {
        ...
        SourceVal = (double *)input_dist + Vnum * k;

        double *DistanceData = (double *)geod_dist + Vnum * k;
    ...
    //     for (int j = 0; j < 4; ++j)
    //     {
    //         std::cout << geod_dist[i][j] << ' ';
    //     }
    //     std::cout << std::endl;
    // }

    double **output = new double*[vnum];

    for (int i = 0; i < vnum; i++)
    {
        output[i] = new double[vnum];
        for (int j = 0; j < vnum; j++)
        {
            output[i][j] = geod_dist[i][j];
        }
    }
...
int main() {
    ...
    double Z[4] = {0.0, 0.0, 0.0, 3.0};
    int TRIV[3][3] = {{0, 1, 2}, {0, 2, 3}, {2, 1 , 3}};

    double **geodesic_dist;

    geodesic_dist = get_fmm_geodesics(X, Y, Z, (int *)TRIV, 4, 3);

    // vector<vector<double>> geod_dist;
    // geod_dist = get_fmm_geodesics(X, Y, Z, (int *)TRIV, 4, 3);

    // double distance[4][4] = {{0, MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE},
    //                          {MAX_DISTANCE, 0, MAX_DISTANCE, MAX_DISTANCE},
    //                          {MAX_DISTANCE, MAX_DISTANCE, 0, MAX_DISTANCE},
    //                          {MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE, 0},
    //                         };


    // double geod_dist[4][4] = {{MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE},
    //                           {MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE},
    //                           {MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE},
    //                           {MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE, MAX_DISTANCE},
    //                          };

    // double geod_dist[4][4] = {};


    // FMM *fmm = new FMM((double *)X, (double *)Y, (double *)Z, (int *)TRIV, 4, 3);

    // int Snum = 4; // Number of sources
    // int Vnum = fmm->GetV();
    ...
    // for (int k = 0; k < Snum; k++)
    //     {
    //         // int n = ((double *)distance)[k] - 1;
    //         // for (int i = 0; i < Vnum; i++)
    //         //     SourceVal[i] = MAX_DISTANCE;
    //         // SourceVal[n] = 0.0;

    //         SourceVal = (double *)distance + Vnum * k;
